@php
    $isSearching = request('search') || request('status') || request('category_id') || request('brand_id');
@endphp
...
<div class="flex flex-col items-center justify-center py-16 px-6 text-center">
    ...
    @else
        <h3 class="font-display text-xl text-primary mb-2">{{ __('messages.empty_products_title') }}</h3>
        <p class="text-sm text-secondary max-w-xs mb-6">{{ __('messages.empty_products_desc') }}</p>
