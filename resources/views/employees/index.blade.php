<x-app-layout>
    ...
    <x-breadcrumbs :items="[['label' => __('messages.employees')]]" />
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        <div>
            <h1 class="font-display text-h1 text-primary">{{ __('messages.employees') }}</h1>
            <p class="text-body text-secondary mt-1">{{ __('messages.manage_employees') }}</p>
        </div>
        <a href="{{ route('employees.create') }}" class="btn-primary self-start">
            {{ __('messages.add_employee') }}
        </a>
    </div>

    <form method="GET" action="{{ route('employees.index') }}" class="flex flex-col sm:flex-row gap-3 mb-6">
        <div class="flex-1">
            <input type="text" name="search" value="{{ request('search') }}" placeholder="{{ __('messages.search_employees_placeholder') }}" class="input-field">
        </div>
        ...
        @if (request('search') || request('status') || request('sort') && request('sort') !== 'latest')
            <a href="{{ route('employees.index') }}" class="inline-flex items-center px-3 text-sm text-secondary hover:text-primary transition-colors">{{ __('messages.clear') }}</a>
        @endif
    </form>
            ...
            <thead>
                <tr class="border-b border-secondary/10">
                    <th class="pb-3 pr-3 w-[3%]">
                        <input type="checkbox" @change="toggleAll()" :checked="selected.length === {{ $employees->count() }} && {{ $employees->count() }} > 0"
            ...
            <tbody>
                @forelse ($employees as $employee)
                    <tr class="border-b border-secondary/5 last:border-0 even:bg-neutral/40 hover:bg-neutral/60 transition-colors"
                        :class="selected.includes({{ $employee->id }}) ? 'bg-tertiary/[0.04] even:bg-tertiary/[0.04]' : ''">
                        <td class="py-3 pr-3 align-middle">
                            <input type="checkbox" value="{{ $employee->id }}" x-model="selected"
                                   class="w-4 h-4 rounded-sm border-secondary/30 text-tertiary focus:ring-tertiary">
                        </td>
                        <td class="py-3 pr-4 text-sm text-secondary align-middle">{{ $loop->iteration + ($employees->currentPage() - 1) * $employees->perPage() }}</td>
                        <td class="py-3 pr-4 text-sm text-primary font-medium align-middle">{{ $employee->name }}</td>
                        <td class="py-3 pr-4 text-sm text-secondary align-middle">{{ $employee->email }}</td>
                        ...
                        <td class="py-3 align-middle">
                            <div class="flex items-center gap-1">
                                <x-tooltip text="{{ __('messages.edit_employee') }}" position="top">
                                    <a href="{{ route('employees.edit', $employee) }}" class="inline-flex items-center gap-1.5 px-3 py-1.5 text-sm text-secondary hover:text-primary hover:bg-neutral rounded-sm transition-colors">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                                        <span>{{ __('messages.edit') }}</span>
                                    </a>
                                </x-tooltip>
                                <x-tooltip text="{{ __('messages.delete') }}" position="top">
                                    <button @click="confirmDelete({{ $employee->id }})" class="inline-flex items-center gap-1.5 px-3 py-1.5 text-sm text-tertiary hover:opacity-80 hover:bg-neutral rounded-sm transition-colors">
    ...
    <div class="sm:hidden space-y-3">
        @forelse ($employees as $employee)
            <div class="card-hover" :class="selected.includes({{ $employee->id }}) ? 'ring-1 ring-tertiary' : ''">
                <div class="flex items-start justify-between mb-2">
                    <div class="flex items-center gap-2">
                        <input type="checkbox" value="{{ $employee->id }}" x-model="selected"
                               class="w-4 h-4 rounded-sm border-secondary/30 text-tertiary focus:ring-tertiary">
                        <h3 class="text-sm font-medium text-primary">{{ $employee->name }}</h3>
                    </div>
                    <span class="font-label text-xs uppercase tracking-[0.08em] {{ $employee->is_active ? 'text-tertiary' : 'text-secondary' }}">
                        {{ $employee->is_active ? __('messages.active') : __('messages.inactive') }}
                    </span>
                ...
                <div class="flex items-center gap-2 pt-3 border-t border-secondary/10 ml-6">
                    ...
                    <button @click="confirmDelete({{ $employee->id }})" title="{{ __('messages.delete') }}" class="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs text-tertiary hover:opacity-80 hover:bg-neutral rounded-sm transition-colors">
    ...
    <x-modal name="confirm-delete" maxWidth="sm">
        <div class="p-6">
            <div class="flex items-center gap-3 mb-4">
                ...
                <div>
                    <h2 class="font-display text-lg text-primary" x-show="deleting === 'bulk'" x-cloak>
                        {{ __('messages.confirm_bulk_delete_employee_title') }}
                    </h2>
                    <h2 class="font-display text-lg text-primary" x-show="deleting !== 'bulk'">
                        {{ __('messages.confirm_delete_employee_title') }}
                    </h2>
                    <p class="text-sm text-secondary mt-0.5" x-show="deleting === 'bulk'" x-cloak>
                        <span x-text="selected.length"></span> {{ __('messages.confirm_bulk_delete_employee_desc') }}
                    </p>
                    <p class="text-sm text-secondary mt-0.5" x-show="deleting !== 'bulk'">
                        {{ __('messages.confirm_delete_employee_desc') }}
                    </p>
                </div>
            </div>

            <form x-show="deleting !== 'bulk'" method="POST" x-bind:action="deleting ? '/employees/' + deleting : '#'">
                @csrf
                @method('DELETE')
                <div class="mt-4">
            ...
            <form x-show="deleting === 'bulk'" method="POST" action="{{ route('employees.bulk-destroy') }}">
                @csrf
                @method('DELETE')
